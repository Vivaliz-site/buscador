...
require_once dirname(__DIR__) . '/includes/buscador-core.php';
...
ais_assert(($deep['openai']['model'] ?? '') === (getenv('BUSCADOR_OPENAI_MODEL') ?: 'gpt-5.6-terra'), 'deep OpenAI model mismatch');
...
ais_assert(($balanced['gemini']['model'] ?? '') === (getenv('BUSCADOR_GEMINI_BALANCED_MODEL') ?: 'gemini-3.5-flash'), 'balanced Gemini model mismatch');
ais_assert(($balanced['gemini']['thinking_level'] ?? '') === 'MEDIUM', 'balanced Gemini thinking must be MEDIUM');
ais_assert(svais_gemini_thinking_config($balanced['gemini']) === ['thinkingLevel' => 'medium'], 'balanced Gemini 3.5 MEDIUM must use thinkingLevel');
$fast = $catalog['fast'];
...
putenv('BUSCADOR_TEST_ANTHROPIC_MODEL=claude-fable-5');
ais_assert(
    svais_non_fable_model('BUSCADOR_TEST_ANTHROPIC_MODEL', 'claude-opus-5') === 'claude-opus-5',
...
$intermediateCoverage = svais_cycle_coverage(
    $intermediateFailure,
    ['openai', 'anthropic', 'gemini'],
    ['research', 'critique', 'converge']
);
ais_assert(
    ($intermediateCoverage['provider_status']['gemini'] ?? '') === 'error',
    'a later Gemini success must not mask an earlier phase failure'
);
ais_assert(
    ($intermediateCoverage['provider_phase_status']['gemini']['research']['status'] ?? '') === 'error',
    'intermediate provider failure must remain observable by phase'
);
ais_assert(
    ($intermediateCoverage['provider_phase_status']['gemini']['research']['failure_class'] ?? '') === 'timeout',
    'intermediate provider failure class must remain observable'
);
ais_assert(
    ($intermediateCoverage['provider_phase_status']['gemini']['critique']['status'] ?? '') === 'ok'
    && ($intermediateCoverage['provider_phase_status']['gemini']['converge']['status'] ?? '') === 'ok',
    'later successful phases must be retained alongside the failure'
);
ais_assert(
    ($intermediateCoverage['complete_provider_coverage'] ?? true) === false,
    'intermediate provider failure must block consensus coverage despite later success'
);
$sourceMissingTranscript = $completeTranscript;
$sourceMissingTranscript[1] = [
    'type' => 'agent_error',
    'ok' => false,
    'provider' => 'anthropic',
    'phase' => 'research',
...
$sourceMissingCoverage = svais_cycle_coverage(
    $sourceMissingTranscript,
    ['openai', 'anthropic', 'gemini'],
    ['research', 'critique', 'converge']
);
...
ais_assert(str_contains($apiSource, '$completeCoverage'), 'API must expose the complete provider/phase coverage gate');
